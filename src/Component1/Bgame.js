...
const Home = ({ children }) => {
  ...
  const filteredGameData = activeItem === "All" ? gameData : gameData.filter(game => game.title.includes(activeItem));
            ...
            {["All", "Win Go", "K3", "TRX","Aviator"].map((item, index) => (
              <Box
                ...
                sx={{
                  ...
                  textAlign: "center",
                  borderRadius: "0.9rem",
                  background: item === activeItem ? "linear-gradient(to bottom, #f5e1a1, #d5aa57)" : "transparent",
